...
<t:page>
  ...
  <script>
    async function getById(name, id) {
      let response = await fetch("${pageContext.request.contextPath}/api/food/portion/" + id, {
        method: "GET",
        headers: {
      ...
      if (response.status === 200) {
        ...
        // Szóközök javítva, hogy szép legyen a kiírás
        alert("A " + name + " ételből rendelt adag: " + obj);
      } else {
        // Felkiáltójel hozzáadva a PDF képe alapján
        alert("Hiba történt!");
      }
    }
  </script>
